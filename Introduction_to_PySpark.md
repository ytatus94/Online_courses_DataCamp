...
* SparkSession
  * `catalog` 屬性列出 cluster 中的所有 data，這個 `catalog` 屬性有許多方法可以呼叫，例如：`spark.catalog.listTables()` 會傳回 cluster 中所有表格的名字
* 同時建立許多 SparkSessions 和 SparkContexts 會造成問題，所以用 `SparkSession.builder.getOrCreate()` 會傳回目前的 SparkSession，若是不存在時就建立一個新的 SparkSession
